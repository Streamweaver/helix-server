...
class RestConflictFilterSet(ReleaseMetadataFilter):
    ...
    RestConflictFilterSet is a class that extends ReleaseMetadataFilter class for filtering Conflict objects based on
    start year and end year.

    Attributes:
    - start_year (django_filters.NumberFilter): A filter for the start year field in Conflict model. It is defined as a
    NumberFilter that uses the method 'filter_start_year' for filtering.
    - end_year (django_filters.NumberFilter): A filter for the end year field in Conflict model. It is defined as a
    NumberFilter that uses the method 'filter_end_year' for filtering.

    Meta:
    - model: The model that the RestConflictFilterSet is associated with is Conflict model.
    - fields: A dictionary of fields to be filtered. In this case, it contains 'id' and 'iso3' fields with the 'iexact'
    lookup.


    Methods:
    - filter_start_year(queryset, name, value): This method is called by the start_year filter to apply filtering based
    on the start year. If the value is not provided, the original queryset is returned. Otherwise, the queryset is
    filtered to include Conflict objects with a year greater than or equal to the provided start year.
    Parameters:
    ...
    - filter_end_year(queryset, name, value): This method is called by the end_year filter to apply filtering based on
    the end year. If the value is not provided, the original queryset is returned. Otherwise, the queryset is filtered
    to include Conflict objects with a year less than or equal to the provided end year.
    Parameters:
        - queryset (QuerySet): The original queryset containing Conflict objects.
...
class RestDisasterFilterSet(ReleaseMetadataFilter):
    ...
    This class is a subclass of ReleaseMetadataFilter and provides custom filters for the Disaster model. It filters the
    queryset based on the event name, start year, and end year.
    ...
    Methods:
    - filter_event_name(queryset, name, value): Filters the queryset by event name. If no value is provided, the
    queryset is returned unchanged. Otherwise, the queryset is filtered to include only records that have an event name
    containing the provided value (case-insensitive).
    - filter_start_year(queryset, name, value): Filters the queryset by start year. If no value is provided, the
    queryset is returned unchanged. Otherwise, the queryset is filtered to include only records that have a year greater
    than or equal to the provided value.
    - filter_end_year(queryset, name, value): Filters the queryset by end year. If no value is provided, the queryset is
    returned unchanged. Otherwise, the queryset is filtered to include only records that have a year less than or equal
    to the provided value.
    - qs(): Returns the filtered queryset after applying an additional filter to include only records with a
    new_displacement greater than 0.
...
class RestDisplacementDataFilterSet(ReleaseMetadataFilter):
    """
    RestDisplacementDataFilterSet class is a filter set used to filter RestDisplacementData objects based on certain
    criteria.

    Attributes:
    - cause: A django_filters.ChoiceFilter that filters the objects based on the cause of displacement. The choices are
    retrieved from the get_name_choices() function using the CRISIS_TYPE_PUBLIC constant.

    - start_year: A django_filters.NumberFilter that filters the objects based on the start year. It only includes
    objects with a year greater than or equal to the provided value.

    - end_year: A django_filters.NumberFilter that filters the objects based on the end year. It only includes objects
    with a year less than or equal to the provided value.

    Methods:
    - filter_start_year(queryset, name, value): This method filters the queryset based on the start year. If no value is
    provided, the queryset is returned unchanged.

    - filter_end_year(queryset, name, value): This method filters the queryset based on the end year. If no value is
    provided, the queryset is returned unchanged.

    - filter_cause(queryset, name, value): This method filters the queryset based on the cause. If no value is provided,
    the queryset is returned unchanged.
        - If the value is 'CONFlict', it includes objects with conflict_new_displacement > 0 or
        conflict_total_displacement > 0.
        - If the value is 'DISASTER', it includes objects with disaster_new_displacement > 0 or
        disaster_total_displacement > 0.

    Properties:
    - qs: This property returns the filtered queryset based on the provided criteria. If no cause is specified, it
    includes objects with conflict or disaster displacement.

    This class extends ReleaseMetadataFilter and uses the DisplacementData model as its Meta model. The fields it
    filters include 'iso3' with 'in' lookup.
    """
...
class PublicFigureAnalysisFilterSet(ReleaseMetadataFilter):
    """
    This class is a filter set used for filtering instances of the PublicFigureAnalysis model. It extends the
    ReleaseMetadataFilter class.

    Attributes:
        cause (django_filters.ChoiceFilter): A choice filter used for filtering instances based on the cause of the
        crisis. It is initialized with a method 'filter_cause' and the choices are obtained from the 'get_name_choices'
        function with the argument 'CRISIS_TYPE_PUBLIC'.

        start_year (django_filters.NumberFilter): A number filter used for filtering instances based on the start year.
        It is initialized with a field_name 'start_year' and a method 'filter_start_year'.

        end_year (django_filters.NumberFilter): A number filter used for filtering instances based on the end year. It
        is initialized with a field_name 'end_year' and a method 'filter_end_year'.

    Meta:
        model (PublicFigureAnalysis): The model class associated with this filter set.

        fields (dict): A dictionary specifying the fields to be filtered. The 'iso3' field is filtered using the 'in'
        lookup.

    Methods:
        filter_start_year(queryset, name, value):
            Filters the queryset based on the start year. If the value is not provided, the queryset is returned as is.
            Otherwise, the queryset is filtered to include only instances with a year greater than or equal to the
            specified value.

        filter_end_year(queryset, name, value):
            Filters the queryset based on the end year. If the value is not provided, the queryset is returned as is.
            Otherwise, the queryset is filtered to include only instances with a year less than or equal to the
            specified value.

        filter_cause(queryset, name, value):
            Filters the queryset based on the cause of the crisis. If the value is not provided, the queryset is
            returned as is. If the value is 'CONFLICT', the queryset is filtered to include only instances with a
            figure_cause equal to the 'CONFLICT' value of the Crisis.CRISIS_TYPE enum. If the value is 'DISASTER', the
            queryset is filtered to include only instances with a figure_cause equal to the 'DISASTER' value of the
            Crisis.CRISIS_TYPE enum.
